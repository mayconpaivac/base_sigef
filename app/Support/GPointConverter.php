...
class GPointConverter
{
    ...
    public function convertLatLngToUtm($latitude, $longitude, $zone = null)
    {
        ...
        if ($LongTemp >= 8 && $LongTemp <= 13 && $latitude > 54.5 && $latitude < 58) { // Special zones for Denmark: http://www.kms.dk/Referencenet/Referencesystemer/UTM_ETRS89/
            $ZoneNumber = 32;
        } elseif ($latitude >= 56.0 && $latitude < 64.0 && $LongTemp >= 3.0 && $LongTemp < 12.0) { // From C++ code
            $ZoneNumber = 32;
        } else {
        ...
        $UTMNorthing = (float)($UTMNorthing);
        $UTMEasting = (float)($UTMEasting);

        return [number_format($UTMEasting, 2, '.', ''), number_format($UTMNorthing, 2, '.', ''), $UTMZone];
    }
